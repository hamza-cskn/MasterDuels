...
public class MatchRoundData {
	...
	/**
	 * @return nexts new round and returns is
	 * there more round? returns false when
	 * last round is finished.
	 */
	public boolean nextRound() {
		if (didAnyTeamWin()) return false;

		//compare as unadded
		return totalRounds > round++;
	}

	public boolean didAnyTeamWin() {
		for (int wins : teamWins.values()) {
			//in 3 rounds game, 2 win means no third round.
