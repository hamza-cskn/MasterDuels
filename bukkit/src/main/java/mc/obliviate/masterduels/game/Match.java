...
public class Match {
    ...
    public Match(final Arena arena, final MatchDataStorage matchDataStorage) {
        ...
        for (final Team.Builder teamBuilder : matchDataStorage.getGameTeamManager().getTeamBuilders()) {
            for (final Member.Builder memberBuilder : teamBuilder.getMemberBuilders()) {
    ...
    public boolean start() {
        if (!Arena.registerGame(arena, this)) {
            players.forEach(p -> MessageUtils.sendMessage(p, "arena-is-busy"));
            this.uninstall();
            return false;
        }
