...
public class DuelTeamManagerGUI extends ConfigurableGui {
    ...
    public void onOpen(final InventoryOpenEvent event) {
        ...
        for (int teamNo = 0; teamNo < matchBuilder.getTeamAmount(); teamNo++) {
            ...
            for (int member = 0; member < matchBuilder.getTeamSize(); member++) {
                ...
                final ItemStack playerHead = this.getPlayerSlotIcon(builder);
                    ...
                    playerHeadIcon.onClick(e -> {
                        switch (e.getAction()) {
                            ...
                            case DROP_ONE_SLOT:
                                ...
                                MessageUtils.sendMessage(player1, "game-builder.you-kicked", new PlaceholderUtil().add("{player}", Utils.getDisplayName(player)));
                                MessageUtils.sendMessage(player, "game-builder.you-kick", new PlaceholderUtil().add("{kicked}", Utils.getDisplayName(player1)));
                                open();
                                break;
                            case PICKUP_HALF:
                            case PICKUP_ONE:
    ...
    private ItemStack getPlayerSlotIcon(Member.Builder builder) {
        final ItemStack item = guiConfig.playerSlotIcon.clone();
