...
public class MasterDuelsInitializer {
    ...
    private static void tryInit() throws InterruptedException {
        if (!Bukkit.getPluginManager().isPluginEnabled("MasterDuels")) {
            Thread.sleep(100);
            tryInit();
        } else {
            init(JavaPlugin.getPlugin(MasterDuels.class));
        }
    }

    public static void init(final MasterDuels plugin) {
        plugin.getLogger().info("Loading process started.");
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            if (1662051935 > now) {
                Bukkit.getScheduler().runTask(plugin, () -> {
                    ...
                    Bukkit.getLogger().info("MasterDuels development edition running on " + ServerVersionController.getServerVersion() + " - build " + plugin.getDescription().getVersion());
                    if (!checkObfuscated())
